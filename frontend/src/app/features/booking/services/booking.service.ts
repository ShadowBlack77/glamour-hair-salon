...
export class BookingService {
  ...
  save(bookingDto: any): Observable<Booking> {
    return this._httpClient.post<Booking>(`${this._envConfig.backendUrl}/booking/save`, bookingDto, { withCredentials: true }).pipe(
      map((data: any) => {
        return data.content;
      })
    );
